...
curl -X GET "$BASE_URL//dictionary/regions" -H "$HEADERS" -o regions.json
...
jq -c '.[]' regions.json | while read region; do
    ...
    curl -X GET "$BASE_URL//dictionary/regions?parent_id=$region_id" -H "$HEADERS" -o "regions_details/region_$region_id.json"
done
...
for region_file in regions_details/*.json; do
    jq -c '.[]' "$region_file" | while read district; do
        ...
        curl -X GET "$BASE_URL//dictionary/mahalla?district_id=$district_id" -H "$HEADERS" -o "districts_details/district_$district_id.json"
    done
done

echo "Barcha so'rovlar muvaffaqiyatli yakunlandi."F
